...
__saveds __interrupt __asm
static struct InputEvent *video_inputhandler (register __a0 struct InputEvent *ie,
...
/* Now initialize Indivision GFX core
  ...
  if (video_is_using_inputhandler) {
    ...
    if ((input_handler = (struct Interrupt *)AllocMem (sizeof(struct Interrupt),
...
void I_ReadScreen (byte* scr)
{
  ...
  if (video_is_directcgx)
    if (video_bitmap_handle != NULL) {
      ...
      CopyMemQuick (base_address + (video_which != 0 ?
                                          SCREENWIDTH * video_oscan_height: 0),
                    scr, SCREENWIDTH * SCREENHEIGHT);
      UnLockBitMap (video_bitmap_handle);
      video_bitmap_handle = NULL;
#if 0
      ReadPixelArray8 (video_window->RPort, 0,
                                              ...
                                              register __a1 APTR data)
{
  ...
  while (ie != NULL) {
    if (ie->ie_Class == IECLASS_RAWKEY) {
      ...
      if (ie->ie_Code < 0x68) {
        event.data1 = xlate_key (ie->ie_Code, ie->ie_Qualifier,
                                 ie->ie_EventAddress);
        D_PostEvent (&event);
      }
