...
; void __asm c2p8 (register __a2 UBYTE *fBUFFER,
...
;                  register __a5 UBYTE *xlate,
;                  register __d4 UWORD force_update);
;
...
; Only convert 32-pixel "units" that changed since last time.
; Update chunky comparison buffer.
; If nothing has changed, signal signals1 and return immediately.
; Wait for previous QBlit() to completely finish (signals2).
; Then launch passes 3 & 4 with QBlit().
; Return immediately after launching passes 3 & 4.
; Signal signals1 (asynchronously) after completion of pass 3.
; Signal signals2 from CleanUp() on completion of QBlit().
; Calling task must wait for signals1 before next call to c2p8().
;
; Approx timing (A4000/030, 320x200x8):
...
;
; (Unimplemented speedup idea: remember locations of the first and last
; differences, then blit only the parts that have changed.)
;
; (Another unimplemented speedup idea: use a "scrambled" chunky buffer.
; Skip pass 1.)
;
; Example usage:
;
...
;	        c2p8 (fBUFFER, fBUFFER_CMP, &RASTPORT->BitMap->Planes[0],
;		      1<<sigbit1, 1<<sigbit2, WIDTH*HEIGHT,
;                     WIDTH/8*LINESTOSKIP, GfxBase);
;	}
...
merge		macro	; \1 data1, \2 data2, \3=shift \4=mask, \5 temp
		move.l	\2,\5
		lsr.l	#\3,\5
		eor.l	\1,\5
		and.l	\4,\5
...
; save arguments
	...
	IFLE depth-6
		move.l	a5,(xlate-mybltnode,a0)
		move.w	d4,(force_update-mybltnode,a0)
	ENDC
...
initpass1loop:
	IFLE depth-6
		tst.w	(force_update)	; force_update?
		bne	go_c2p
	ENDC
		cmpm.l	(a2)+,(a3)+
		bne	fix1
...
go_c2p:		movem.l	(a2)+,d0-d3	; AaBbCcDd EeFfGgHh IiJjKkLl MmNnOoPp
	...
	IFGT depth-6
		merge	d0,d2,8,d5,d4	; d0=AaIiCcKk d2=BbJjDdLl
		merge	d1,d3,8,d5,d4	; d1=EeMmGgOo d3=FfNnHhPp
	ELSE
		moveq	#0,d4
