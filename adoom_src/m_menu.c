...
void M_DrawEmptyCell(menu_t* menu, int item)
{
    V_DrawPatchInDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, W_CacheLumpName("M_CELL1", PU_CACHE));
}

void M_DrawSelCell(menu_t* menu, int item)
{
    V_DrawPatchInDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 0, W_CacheLumpName("M_CELL2", PU_CACHE));
}

void M_StartMessage(char* string, void (*routine)(int response),
                    /*  void*		routine, */
                    boolean input)
{
...
void M_Drawer(void)
{
    ...
    for (i = 0; i < max; i++) {
        if (currentMenu->menuitems[i].name[0])
            V_DrawPatchInDirect(x, y, 0, W_CacheLumpName(currentMenu->menuitems[i].name, PU_CACHE));
        y += LINEHEIGHT;
    }
