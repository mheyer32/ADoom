...
void R_GenerateComposite(REGD0(int texnum))
{
    ...
    for (i = 0, patch = texture->patches; i < texture->patchcount; i++, patch++) {
        ...
        for (; x < x2; x++) {
            ...
            patchcol = (column_t*)((byte*)realpatch + SWAPLONG(realpatch->columnofs[x - x1]));
...
void R_InitTextures(void)
{
    ...
    for (i = 0; i < numtextures; i++, directory++) {
        ...
        texture = textures[i] =
            Z_Malloc(sizeof(texture_t) + sizeof(texpatch_t) * (SWAPSHORT(mtexture->patchcount) - 1), PU_STATIC, 0);
