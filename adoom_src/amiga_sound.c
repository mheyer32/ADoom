...
static struct chip_cache_info chip_cache_info[CHIP_CACHE_SIZE] = {
    {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1},
    {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}, {-1, 0, NULL, -1}};
...
static UWORD freqs[256] = {
    ...
    4635,  4686,  4737,  4789,  4841,  4893,  4947,  5001,  5055,  5110,  5166,  5222,  5279,  5336,  5394,  5453,
    5513,  5573,  5633,  5695,  5757,  5819,  5883,  5947,  6011,  6077,  6143,  6210,  6278,  6346,  6415,  6485,
    6556,  6627,  6699,  6772,  6846,  6920,  6996,  7072,  7149,  7227,  7305,  7385,  7465,  7547,  7629,  7712,
    7796,  7881,  7967,  8053,  8141,  8230,  8319,  8410,  8501,  8594,  8688,  8782,  8878,  8975,  9072,  9171,
    9271,  9372,  9474,  9577,  9681,  9787,  9893,  10001, 10110, 10220, 10331, 10444, 10558, 10673, 10789, 10906,
    11025, 11145, 11266, 11389, 11513, 11638, 11765, 11893, 12023, 12154, 12286, 12420, 12555, 12692, 12830, 12970,
    13111, 13254, 13398, 13544, 13691, 13841, 13991, 14144, 14298, 14453, 14611, 14770, 14931, 15093, 15258, 15424,
    15592, 15761, 15933, 16107, 16282, 16459, 16639, 16820, 17003, 17188, 17375, 17564, 17756, 17949, 18144, 18342,
    18542, 18744, 18948, 19154, 19363, 19574, 19787, 20002, 20220, 20440, 20663, 20888, 21115, 21345, 21578, 21812,
    22050, 22290, 22533, 22778, 23026, 23277, 23530, 23787, 24046, 24308, 24572, 24840, 25110, 25384, 25660, 25940,
    26222, 26508, 26796, 27088, 27383, 27681, 27983, 28287, 28595, 28907, 29221, 29540, 29861, 30187, 30515, 30848,
...
void I_InitSound(void)
{
    ...
    if (DoomSndBase == NULL) {
        ...
        if ((audio_mp = CreatePort(NULL, 0)) == NULL ||
            (audio_io = (struct IOAudio *)AllocMem(sizeof(struct IOAudio), MEMF_PUBLIC | MEMF_CLEAR)) == NULL)
        ...
        for (i = 0; i < numChannels; i++) {
            ...
            if ((c->audio_mp = CreatePort(NULL, 0)) == NULL ||
                (c->audio_io = (struct IOAudio *)AllocMem(sizeof(struct IOAudio), MEMF_PUBLIC | MEMF_CLEAR)) == NULL)
                I_Error("CreatePort() or AllocMem() failed");
            *c->audio_io = *audio_io;
    ...
    for (i = 0; i < 256; i++) {
        if (changepitch)
            period_table[i] = ((clock_constant << 1) + freqs[i]) / (((ULONG)freqs[i]) << 1);
        else
...
void I_ShutdownSound(void)
{
    ...
    if (audio_is_open) {
        for (i = 0; i < numChannels; i++)
            I_StopSound(i);
        audio_io->ioa_Request.io_Unit = (struct Unit *)((1 << numChannels) - 1); /* free numChannels channels */
...
int I_StartSound(int id, int cnum, int vol, int sep, int pitch, int priority)
{
    ...
    if (DoomSndBase != NULL) {
        I_StopSound(cnum);
        Sfx_Start(S_sfx[id].data, cnum, changepitch ? freqs[pitch] : 11025, vol << 3, sep, lengths[id]);
        return cnum;
    }
