...
static void SER_PacketGet(void)
{
...

    {
        static int first = 1;
        if (first)
            printf("len=%d:p=[0x%x 0x%x] \n", SER_get_len2, *(int *)&SER_get_sw, *((int *)&SER_get_sw + 1));
        first = 0;
    }
...
void SER_InitNetwork(int i)
{
    ...
    if ((SER_writelen_mp = CreatePort(NULL, 0)) == NULL || (SER_readlen_mp = CreatePort(NULL, 0)) == NULL ||
        (SER_write_mp = CreatePort(NULL, 0)) == NULL || (SER_read_mp = CreatePort(NULL, 0)) == NULL ||
        (SER_writelen_io = (struct IOExtSer *)CreateExtIO(SER_writelen_mp, sizeof(struct IOExtSer))) == NULL ||
        (SER_readlen_io = (struct IOExtSer *)CreateExtIO(SER_readlen_mp, sizeof(struct IOExtSer))) == NULL ||
        (SER_write_io = (struct IOExtSer *)CreateExtIO(SER_write_mp, sizeof(struct IOExtSer))) == NULL ||
        (SER_read_io = (struct IOExtSer *)CreateExtIO(SER_read_mp, sizeof(struct IOExtSer))) == NULL)
        I_Error("Can't create port");

    unit = atoi(myargv[i + 3]);
    speed = atoi(myargv[i + 4]);

    if (M_CheckParm("-no7wire"))
        serflags = SERF_XDISABLED | SERF_RAD_BOOGIE;
    else
        serflags = SERF_XDISABLED | SERF_RAD_BOOGIE | SERF_7WIRE;

    SER_write_io->io_SerFlags = serflags;
...
static void IPX_PacketGet(void)
{
    ...
    /*
      ...
      for (i = 0; i < doomcom->datalength; i++)
        printf (" %02x", ((UBYTE *)&doomcom->data)[i]);
      printf ("\n");
    */
...
static void IPX_LookForNodes(int numnetnodes)
{
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            /*
                  printf ("Received from %d at %d, gameid = %d, drone = %d, nodesfound = %d, nodeswanted = %d\n",
                          doomcom->remotenode, IPX_remotetime, dest->gameid, dest->drone,
