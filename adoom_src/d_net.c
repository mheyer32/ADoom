...
// FIXME -endianess?
    ...
    for (i = 0; i < netbuffer->numtics; i++) {
        t = &netbuffer->cmds[i];
        c += ((i << 1) + 2) * (((unsigned char)t->forwardmove) + (((unsigned char)t->sidemove) << 8) +
                               (((unsigned short)t->angleturn) << 16));
        c += ((i << 1) + 3) * (((unsigned short)t->consistancy) + (((unsigned char)t->chatchar) << 16) +
...
boolean HGetPacket(void)
{
    ...
    if (NetbufferChecksum() != (netbuffer->checksum & NCMD_CHECKSUM)) {
        ...
        fprintf(stderr, "%08x", ((unsigned long *)&netbuffer->retransmitfrom)[0]);
        for (i = 0; i < netbuffer->numtics; i++)
            fprintf(stderr, " %08x %08x", ((unsigned long *)&netbuffer->cmds[0])[2 * i],
