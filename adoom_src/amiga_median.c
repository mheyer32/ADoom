...
static FAR UWORD hist[NSIDE][NSIDE][NSIDE]; /* 3D histogram (cube) */
...
void median_cut(UBYTE *palette, ULONG *table, UBYTE *xlate)
{
    ...
    for (this_idx = 1; this_idx < max_nodes; this_idx++) {
        ...
        for (idx = 0; idx < this_idx; idx++)
            if (node[idx].badness > max_badness || (node[idx].badness == max_badness && node[idx].n > max_n)) {
