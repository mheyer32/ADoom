...
int DE_Search(char *arg, char **list)
{
    int i, l;
...
int DE_Hack(char *line)
{
    ...
    if (choice < 0 || choice >= NUM_KEYWORD)
        return 0;

    arg = strstr(line, Keywords[choice]) + strlen(Keywords[choice]);
