...
void WI_drawLF(void)
{
    ...
    // draw <LevelName>
    V_DrawPatchInDirect((320 - SWAPSHORT(lnames[wbs->last]->width)) / 2, y, FB, lnames[wbs->last]);

    // draw "Finished!"
    y += (5 * SWAPSHORT(lnames[wbs->last]->height)) / 4;

    V_DrawPatchInDirect((320 - SWAPSHORT(finished->width)) / 2, y, FB, finished);
}
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatchInDirect((320 - SWAPSHORT(entering->width)) / 2, y, FB, entering);

    // draw level
    y += (5 * SWAPSHORT(lnames[wbs->next]->height)) / 4;

    V_DrawPatchInDirect((320 - SWAPSHORT(lnames[wbs->next]->width)) / 2, y, FB, lnames[wbs->next]);
}
...
void WI_drawDeathmatchStats(void)
{
    ...
    V_DrawPatchInDirect(DM_TOTALSX - SWAPSHORT(total->width) / 2, DM_MATRIXY - WI_SPACINGY + 10, FB, total);
    ...
    for (i = 0; i < MAXPLAYERS; i++) {
        if (playeringame[i]) {
            V_DrawPatchInDirect(x - SWAPSHORT(p[i]->width) / 2, DM_MATRIXY - WI_SPACINGY, FB, p[i]);

            V_DrawPatchInDirect(DM_MATRIXX - SWAPSHORT(p[i]->width) / 2, y, FB, p[i]);

            if (i == me) {
                V_DrawPatchInDirect(x - SWAPSHORT(p[i]->width) / 2, DM_MATRIXY - WI_SPACINGY, FB, bstar);

                V_DrawPatchInDirect(DM_MATRIXX - SWAPSHORT(p[i]->width) / 2, y, FB, star);
            }
        } else {
...
void WI_drawNetgameStats(void)
{
    ...
    // draw stat titles (top line)
    V_DrawPatchInDirect(NG_STATSX + NG_SPACINGX - SWAPSHORT(kills->width), NG_STATSY, FB, kills);

    V_DrawPatchInDirect(NG_STATSX + 2 * NG_SPACINGX - SWAPSHORT(items->width), NG_STATSY, FB, items);

    V_DrawPatchInDirect(NG_STATSX + 3 * NG_SPACINGX - SWAPSHORT(secret->width), NG_STATSY, FB, secret);

    if (dofrags)
        V_DrawPatchInDirect(NG_STATSX + 4 * NG_SPACINGX - SWAPSHORT(frags->width), NG_STATSY, FB, frags);
...
void WI_drawStats(void)
{
    ...
    V_DrawPatchInDirect(SP_TIMEX, SP_TIMEY, FB, timeDisplay);
...
void WI_loadData(void)
{
    ...
    // "time"
    timeDisplay = W_CacheLumpName("WITIME", PU_STATIC);
...
void WI_unloadData(void)
{
    ...
    Z_ChangeTag(timeDisplay, PU_CACHE);
